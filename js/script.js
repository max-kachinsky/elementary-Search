...
$(function($) {
	...
	$('.cookies-enabled').click(function() {
		...
		} else if (action === 'https://www.google.ru/search') {
			$.cookie('default_search', 'Google', { expires: 365, path: '/' });
		} else if (action === 'https://www.bing.com/search') {
			$.cookie('default_search', 'Bing', { expires: 365, path: '/' });
		};
	});
	...
	$('.change-to-duckduckgo').click(function() {
		...
		$('.current-search').html('В данный момент Вы используете DuckDuckGo.');
		if (cookies === 'enabled') {
			$.cookie('default_search', 'DuckDuckGo', { expires: 365, path: '/' });
		};
	});	$('.change-to-google').click(function() {
		$('#search').attr('action', 'https://www.google.ru/search');
		$('.current-search').html('В данный момент Вы используете Google.');
		if (cookies === 'enabled') {
			$.cookie('default_search', 'Google', { expires: 365, path: '/' });
		};
	});
	$('.change-to-bing').click(function() {
		$('#search').attr('action', 'https://www.bing.com/search');
		$('.current-search').html('В данный момент Вы используете Bing');
		if (cookies === 'enabled') {
			$.cookie('default_search', 'Bing', { expires: 365, path: '/' });
		};
	});
